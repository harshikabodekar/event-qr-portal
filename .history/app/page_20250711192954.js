...
export default function HomePage() {
  ...
  const checkedIn = students.filter(s => s.checkin_status).length;
  ...
  useEffect(() => { if (selectedEvent) fetchStudents(); }, [selectedEvent]);
  async function fetchStudents() {
    const { data } = await supabase.from('students').select('*').eq('event_id', selectedEvent);
    setStudents(data || []);
  }
  ...
  async function handleToggleCheckin(student) {
    const { error } = await supabase.from('students').update({ checkin_status: !student.checkin_status }).eq('id', student.id);
            ...
            <tbody>
              {students.map(student => (
                <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-2 text-center">
                    <button onClick={() => handleToggleCheckin(student)} className={`rounded px-3 py-1 font-semibold ${student.checkin_status ? 'bg-green-600 text-white' : 'bg-gray-300 text-gray-700'}`}>
                      {student.checkin_status ? 'Checked-in' : 'Pending'}
                    </button>
                  </td>
