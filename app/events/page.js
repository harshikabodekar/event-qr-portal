...
export default function EventsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Toaster />
      <h1 className="text-2xl font-bold mb-6 text-black">Events</h1>
      ...
      <ul className="space-y-2">
        {events.map(ev => (
          <li key={ev.id} className="p-4 bg-white rounded shadow flex justify-between items-center">
            <span className="text-black font-medium">{ev.name} <span className="text-gray-500 text-sm">({ev.date})</span></span>
            <a href={`/register/${ev.id}`} className="text-blue-600 underline hover:text-blue-800">Register</a>
          </li>
        ))}
      </ul>
